...
import { sendOrderBankDetailsEmail } from '@/services/orderBankDetailsService';
...
export async function POST(request) {
  try {
      ...
      try {
        ...
        if (!soErr && so) {
          ...
          order = {
            ...
            payment_status: '10% Paid / 90% Due',
            order_status: so.current_state || 'dock_inspection',
            delivery_option: 'deliver',
            delivery_address: deliveryAddr || so.buyer?.city || 'Delivery Warehouse',
            delivery_date: so.estimated_delivery_days ? `${so.estimated_delivery_days} days` : 'Pending Dispatch',
        ...
        console.warn('Supabase lookup for 90% payment dispatch notice:', dbErr.message);
      }
    }
    ...
    const result = await sendOrderBankDetailsEmail(order, options);

    if (!result.success) {
      return NextResponse.json({ error: result.error || 'Failed to dispatch 90% bank details email' }, { status: 500 });
    }

    return NextResponse.json({
      success: true,
      message: `✓ 90% Balance Banking Details [A/C: 20521984403, IFSC: SBIN0011514] successfully emailed to ${result.recipientEmail}!`,
    ...
    console.error('Error in /api/admin/orders/send-bank-details:', err);
